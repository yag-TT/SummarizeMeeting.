...
from summarize_meeting.processing.audio_enhancement import MODEL_NAME


class AudioEnhancementController(QObject):
    ...
    def start(self, session_directory: Path) -> None:
        ...
        with self._lock:
            if self._running:
                raise RuntimeError("マイク音声の改善は既に実行中です")
            self._cancel_requested = False
            self._running = True
        try:
            ...
            self._clear_process()
            raise RuntimeError(f"音声改善状態を保存できません: {exc}") from exc
        thread = threading.Thread(
            target=self._run_worker,
            args=(session_directory, state),
            name="audio-enhancement-job",
            daemon=True,
        )
        try:
            thread.start()
    ...
    def _run_worker(self, session_directory: Path, state: AnalysisJobState) -> None:
        ...
        command = [
            sys.executable,
            "-m",
            "summarize_meeting.processing.audio_enhancement_worker",
            "--session",
        ...
        if canceled:
            ...
            message = f"マイク音声の改善に失敗しました: {detail}"
            persistence_error = self._persist_terminal(
                session_directory,
                state,
                AnalysisJobStatus.FAILED,
