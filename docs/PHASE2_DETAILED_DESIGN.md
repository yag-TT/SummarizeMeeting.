...
1. 物理マイク・実PC再生音声によるGUI経由の短時間正常系試験を行う

仮想Windows音声デバイスを使った2トラック録音から自動文字起こしまでの経路と、1時間の2トラックを使った処理時間、GPUメモリ、segment構造の測定は完了した。実会議音声の認識精度は、評価音声と正解文を用意した後に別途評価する。再配布ライセンス確認は、配布しない方針のため残作業に含めない。
