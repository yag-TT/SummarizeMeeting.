...
最終WAVの検証成功後に `audio/manifest.json` の保存だけが `OSError` になった場合は、検証済みWAVを削除せず `FINALIZE_FAILED` を記録し、Sessionを `INTERRUPTED` としてterminal通知まで継続する。次回起動時はmanifestなしの最終WAVをheaderとPCM末尾読取りで自己整合性検証し、重複したrecovered WAVを生成しない。PREPARING失敗後のcleanup manifest保存も例外をcleanup failureへ変換し、`FAILED_TO_START` とterminal eventの確定を妨げない。

録音中にユーザーがアプリウィンドウを閉じた場合は、「録音を終了してアプリを閉じますか？」という確認を表示する。キャンセル時は録音を継続する。確認後の終了、またはOS終了要求を受けた場合は同じ停止処理を通す。OS終了要求はQtの `commitDataRequest` で検出し、UIを確認なし終了モードへ切り替えて停止を要求する。OS終了時には確認ダイアログを表示せず、最大4秒だけterminal eventを待ってbest-effortでfinalizeする。時間内に完了しない場合もOS終了をcancelせず、次回起動時に残ったsegmentを復旧する。強制終了しかできない状態ではセッションを `INTERRUPTED` として残す。
